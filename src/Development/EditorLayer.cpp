...
bool EditorLayer::OnEvent(Event& e) {
	...
	else if (Input::KeyPressed(KEY_LEFT_CONTROL) && Input::KeyPressed(KEY_C)) {
		CopyEntity();
		Scene::s_msgbatch.Add(MessageType::info, "Entity Copied.");
	}
	else if (Input::KeyPressed(KEY_LEFT_CONTROL) && Input::KeyPressed(KEY_V)) {
		PasteEntity();
	...
	for (auto e : view) {
		...
		if (cc.Primary) {
			if (cc.cameratype == CameraType::Orthographic) {
				...
				cc.camerap.Update();
				camproj = cc.camerap.viewprojection * glm::lookAt(tc.position, tc.position + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0, 1.0f, 0.0f));
			}
			break;
		}
	}
...
void EditorLayer::PasteEntity() {
	if (s_CopiedEntity.nc.has_value()) {
		Entity entity = selectedscene->CreateEntity(s_CopiedEntity.nc.value().name);
		// copy position
