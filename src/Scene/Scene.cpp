...
void Scene::Update(float ts) {
	...
	for (auto e : view) {
		...
		if (cc.Primary) {
			...
			if (cc.cameratype == CameraType::Orthographic) {
				...
				Application::m_Shader->Bind();
				Application::m_Shader->SetFloat("w", 1.0f);
			}
			else {
				cc.camerap.Update();
				cameramatrix = cc.camerap.viewprojection * glm::lookAt(transform.position, transform.position + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
				Application::m_Shader->Bind();
				Application::m_Shader->SetFloat("w", 1.0f);
			}
	...
	if (cameraexist) {
		...
		Renderer2D::DrawQuad(glm::vec3(0.0f, 0.0f, 0.0f), { 3.0f, 3.0f }, { 1.0f, 0.0f, 0.0f, 1.0f });;
		for (auto e : entities) {
			if (e.HasComponent<SpriteRendererComponent>() && e.GetComponent<NameComponent>().active) {
				...
				if (sc.type == 0)
					...
					Renderer2D::DrawQuad(transform.getmatrix(), sc.m_subtexture, sc.opacity);
				}
		...
		if (!Application::EditorMode) {
			...
			Renderer2D::UseShader(Application::particle_shader);
			for (auto e : entities) {
		...
		Renderer2D::UseShader(Application::m_Shader);
